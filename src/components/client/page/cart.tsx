...
useEffect(() => {
  if (!userId) return; // tránh gọi API sớm khi chưa có user
  getProductCart();
}, [userId]);
  ...
  const handleQuantityChange = (productId: number, color: string, storage: string, delta: number) => {
        ...
        <div className="overflow-x-auto rounded-lg shadow-lg">
              ...
              {cartItems.map((item, index) => (
                <tr key={item.id} className="border-t hover:bg-gray-50 transition-all">
                  ...
                  <td className="p-4 flex items-center gap-4">
                    <img src={item.productId.image} alt={item.productId.name} className="w-16 h-16 rounded-lg border object-cover" />
                  ...
                  <td className="text-center p-4">
                    <button onClick={() => handleRemove(item.id)} className="text-sm text-red-600 font-semibold px-3 py-1 rounded border border-red-600 hover:bg-red-600 hover:text-white transition-colors duration-200">
                      Xóa
                    </button>
                  </td>
                </tr>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mt-6 border-t pt-6 px-4 pb-6">
            ...
            <div className="text-right space-y-2">
              ...
              <button onClick={handleCheckout} className="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 transition font-semibold">
                Đặt hàng
              </button>
            </div>
          </div>
